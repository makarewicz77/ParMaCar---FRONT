                        ...
                        <>
                          ...
                          /> :
                            <img
                              ...
                              className="products_list-item-image"
                            />}
                          <h5 className="products_list-item-name">{product.name}</h5>
                        </>
                      }
                    >
                      {product.description.length > 55 ?
                        <Popover content={fullDescription(product.description)}><div>
                          <p className="products_list-item-producer">{product.producer}</p>
                          <p className="products_list-item-short_description">{cutDescription(product.description)}</p>
                        </div>

                        </Popover> :
                        <div className={product.description.length > 16 ? "product_div_long" : "product_div_short"}>
                          <p className="products_list-item-producer">{product.producer}</p>
                          <p className="products_list-item-short_description">{product.description}</p>
                        </div>}
                      <h5 className="products_list-item-price">{product.gross} {t("product.value")}</h5>
                    </Card>
                  </Col>
                </div>
              );
